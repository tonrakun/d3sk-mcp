...
impl D3skServer {
    ...
    fn apply(&self, res: pc::Res) -> String {
        let (r, e) = res;
        if let Some(msg) = e { self.set_error(msg); }
        r
    }

    fn apply_web(&self, res: web::Res) -> String {
        let (r, e) = res;
        if let Some(msg) = e { self.set_error(msg); }
        r
    }
...
impl D3skServer {
    ...
    #[tool(description = "Execute multiple tools in sequence. steps_json: JSON array string of [{tool,...args}]. on_error: 'stop'|'continue'")]
    async fn batch(&self, Parameters(p): Parameters<BatchParams>) -> String {
        ...
        for step in &steps {
            ...
            let failed = result.starts_with('E');
            results.push(result);
    ...
    #[tool(description = "List running windowed apps. Returns '<pid>,<name>,<title>' lines")]
    ...
    async fn browser_connect(&self, Parameters(p): Parameters<BrowserConnectParams>) -> String {
        self.apply_web(web::browser_connect(&self.web_sessions, p.port, p.browser).await)
    }
    ...
    async fn browser_open(&self, Parameters(p): Parameters<BrowserOpenParams>) -> String {
        self.apply_web(web::browser_open(&self.web_sessions, p.browser, p.profile).await)
    }
    ...
    #[tool(description = "Get current URL and title. Returns '<url>,<title>'")]
    async fn get_url(&self, Parameters(p): Parameters<SessionParam>) -> String {
        self.apply_web(web::get_url(&self.web_sessions, p.session_id).await)
    }

    // ── web: DOM ──────────────────────────────────────────────────────────────

    #[tool(description = "Get DOM. interactive_only: returns 'tag,selector,text' lines for interactive elements")]
    ...
    async fn click(&self, Parameters(p): Parameters<SelectorParams>) -> String {
        self.apply_web(web::click(&self.web_sessions, p.selector, p.session_id).await)
    }
    ...
    async fn web_select(&self, Parameters(p): Parameters<SelectParams>) -> String {
        self.apply_web(web::select(&self.web_sessions, p.selector, p.value, p.session_id).await)
    }

    #[tool(description = "Set checkbox/radio checked state")]
    async fn check(&self, Parameters(p): Parameters<CheckParams>) -> String {
        self.apply_web(web::check(&self.web_sessions, p.selector, p.checked, p.session_id).await)
    }
    ...
    async fn web_scroll(&self, Parameters(p): Parameters<WebScrollParams>) -> String {
        self.apply_web(web::scroll_page(&self.web_sessions, p.selector, p.delta_x, p.delta_y, p.unit, p.session_id).await)
    }
    ...
    #[tool(description = "Browser screenshot. Returns '<base64>,<w>,<h>'")]
    ...
    #[tool(description = "List open tabs. Returns '<id>,<url>,<title>' lines")]
    async fn tab_list(&self, Parameters(p): Parameters<SessionParam>) -> String {
        self.apply_web(web::tab_list(&self.web_sessions, p.session_id).await)
    }

    #[tool(description = "Open new tab. Returns tab id")]
    async fn tab_new(&self, Parameters(p): Parameters<TabNewParams>) -> String {
        self.apply_web(web::tab_new(&self.web_sessions, p.url, p.session_id).await)
    }

    #[tool(description = "Switch active tab by id")]
    async fn tab_switch(&self, Parameters(p): Parameters<TabIdParams>) -> String {
        self.apply_web(web::tab_switch(&self.web_sessions, p.id, p.session_id).await)
    }

    #[tool(description = "Close tab by id")]
    async fn tab_close(&self, Parameters(p): Parameters<TabIdParams>) -> String {
        self.apply_web(web::tab_close(&self.web_sessions, p.id, p.session_id).await)
    }

    // ── web: cookie ───────────────────────────────────────────────────────────

    #[tool(description = "Get cookies. Returns '<name>,<value>,<domain>' lines")]
