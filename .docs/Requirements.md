...
| # | 変更内容 |
|---|---|
| 1 | `E409` エラーコードを廃止（未使用のため削除） |
| 2 | `screenshot.target` / `mouse_click.action` / `mouse_scroll.unit` / `web_scroll.unit` に `serde(default)` を適用（省略時はデフォルト値が自動適用） |
| 3 | `dispatch_tool` を `d!` マクロ＋serde デシリアライズに統一（`#[tool]` メソッドと実装経路を共有） |
| 4 | `navigate` のセッション二重検索を解消（1回のルックアップで action / url 両分岐を処理） |
| 5 | `with_session!` マクロ追加（`tab_list` / `tab_new` / `tab_switch` / `tab_close` に適用） |
| 6 | `install.ps1` に try/catch を追加しエラー時・成功時ともにウィンドウを保持 |
| 7 | Clippy 警告を全解消・未使用依存（`thiserror` / `tokio-stream`）を削除 |
| 8 | CI に `cargo clippy -- -D warnings` ステップを追加 |
...
| # | 変更内容 |
|---|---|
| 1 | `get_dom` `interactive_only: true` のレスポンスをJSON化（`[{tag,selector,text}]`） |
| 2 | `screenshot` の `scale` 上限を `2.0` に拡大（`web_screenshot` と統一） |
| 3 | `shell` に `cwd?` パラメータ追加（作業ディレクトリ指定） |
| 4 | `shell` / `wait_for` の `timeout_ms` デフォルト値を明記（30000ms） |
| 5 | `mouse_drag` に `duration_ms?` 追加（ドラッグ所要時間制御、デフォルト100ms） |
| 6 | `type_text` / `type`（web）に `delay_ms?` 追加（文字間遅延制御） |
| 7 | `navigate` に `action: "back"\|"forward"\|"reload"` 追加（URL代替） |
| 8 | `wait_for` の `state` に `"attached"\|"detached"` 追加 |
| 9 | `web_screenshot` の `selector?` を実装（要素スクリーンショット） |
| 10 | `file_exists` ツール追加 |
| 11 | `file_copy` ツール追加 |
| 12 | `get_text` ツール追加（要素のテキスト取得、軽量） |
| 13 | `get_attr` ツール追加（要素の属性値取得） |
| 14 | `dialog_handle` ツール追加（alert/confirm/promptの処理） |
| 15 | `cookie_set` ツール追加 |
...
#### `mouse_drag`
```
Request:  { from_x: i32, from_y: i32, to_x: i32, to_y: i32, duration_ms?: u64 }  // duration_ms デフォルト 100
Response: "ok" | "E<code>"
```
...
#### `type_text`
```
Request:  { text: string, delay_ms?: u64 }  // delay_ms: 文字間遅延ms（省略時は遅延なし）
Response: "ok" | "E<code>"
```
...
#### `shell`
```
Request:  { cmd: string, timeout_ms?: u64, cwd?: string }  // timeout_ms デフォルト 30000
...
#### `navigate`
`url` と `action` はどちらか一方を指定する。`wait` の挙動：`goto` は `load` イベントまで待機するため、`"load"` と `"domcontentloaded"` は同等。`"networkidle"` のみ `wait_for_navigation` による追加待機を行う。
```
Request:  {
  url?: string,                                         // URLへ遷移
  action?: "back" | "forward" | "reload",               // 履歴操作（url代替）
...
Response: [{ tag: string, selector: string, text: string }]  // JSON
...
#### `get_text`
セレクタにマッチした要素のテキストコンテンツを返す。DOM全体取得より軽量。
```
Request:  { selector: string, session_id?: string }
Response: "<text>" | "E<code>"
```

#### `get_attr`
セレクタにマッチした要素の属性値を返す。
```
Request:  { selector: string, attr: string, session_id?: string }
Response: "<value>" | "E<code>"
```
...
#### `type`
```
Request:  { selector: string, text: string, clear?: bool, delay_ms?: u64, session_id?: string }
Response: "ok" | "E<code>"
```
...
Request:  {
  selector: string,
  timeout_ms?: u64,                                           // デフォルト 30000
  state?: "visible" | "hidden" | "attached" | "detached",    // デフォルト "visible"
  session_id?: string
}
Response: "ok" | "E<code>"
```
...
#### `web_screenshot`
`scale` を指定した場合はスクリーンショット取得後にLanczos3でリサイズする（0.1〜2.0）。`selector` を指定した場合はその要素の領域のみをキャプチャする。
```
...
#### `dialog_handle`
ページ上に表示された alert / confirm / prompt ダイアログを処理する。
```
Request:  { action: "accept" | "dismiss", text?: string, session_id?: string }
// text: prompt ダイアログへの入力値（accept 時のみ有効）
Response: "ok" | "E<code>"
```
...
#### `cookie_set`
```
Request:  { name: string, value: string, url?: string, domain?: string, path?: string, session_id?: string }
Response: "ok" | "E<code>"
```
